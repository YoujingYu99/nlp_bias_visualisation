...
from bias_visualisation_app.utils.functions import get_text_url, get_text_file, generate_list, list_to_dataframe, \
    SVO_analysis, modifier_analysis, \
...
def detect_dataframe():
    if request.method == "POST":
        try:
            ...
            dataframe_modifier = request.files['modifier_file']
        ...
        input_dataframe_modifier = pd.read_csv(dataframe_modifier)
        save_obj_user_uploads(input_dataframe_modifier, name="modifier_dataframe_user_uploads")
...
def analysis():
    ...
    input_modifier_dataframe = load_obj_user_uploads(name="modifier_dataframe_user_uploads")
    ...
    return render_template('analysis.html', data_fm_tot=female_tot_df, data_m_tot=male_tot_df,
                           ...
                           data_m_obj_verb=male_obj_df, data_fm_modifier=female_modifier_df, data_m_modifier=male_modifier_df,
                           wordtype_data=[{'type': 'nouns'}, {'type': 'adjectives'}, {'type': 'intransitive_verbs'}, {'type': 'subject_verbs'},
                                          {'type': 'object_verbs'}, {'type': 'modifiers'}],
...
def query():
    if request.method == 'POST':
        ...
        dataframe_to_display = df_based_on_question(str(select_wordtype), str(select_gender), view_df,
                                                    input_SVO_dataframe, input_modifier_dataframe)
...
@app.route('/uploads/<path:filename>', methods=['GET', 'POST'])
def download_modifier(filename='modifier_dataframe.csv'):
    uploads = path.join(path.dirname(__file__), "static\\", filename)
    print(uploads)
    # return send_from_directory(directory=uploads, filename=filename)
    return send_file(uploads, as_attachment=True)
